...
class SaaSWebSettingsDashboard(WebSettingsDashboard):
    ...
    def web_settings_dashboard_data(self, **kw):
        ...
        cur_users = request.env['res.users'].search_count([('share', '=', False), ('id', 'not in', [SUPERUSER_ID])])
        ...
        datetime_obj = expiration_datetime and datetime.strptime(expiration_datetime, DEFAULT_SERVER_DATE_FORMAT)
        ...
        result.update({'saas': {'cur_users': cur_users,
                                'max_users': max_users,
                                'expiration_datetime': datetime_obj and datetime_obj.strftime(DEFAULT_SERVER_DATE_FORMAT),
