...
class HrEmployeeFamilyInfo(models.Model):
    ...
    member_name = fields.Char(string='Name', readonly=False, related='employee_ref.name', store=True)
    ...
    employee_id = fields.Many2one(string="Employee", readonly=True, help='Select corresponding Employee', comodel_name='hr.employee',
                                  invisible=True)
    relation = fields.Selection([('father', 'Father'),
                                 ...
                                 ('sister', 'Sister'),
                                 ('daughter', 'Daughter'),
                                 ('son', 'Son')], string='Relationship', help='Relation with employee')
    member_contact = fields.Char(string='Contact No', readonly=False) # Removed related='employee_ref.personal_mobile' and store=True
...
class HrEmployee(models.Model):
    ...
    ############### Social info ##############
    employee_fb = fields.Char(string='Facebook ID')
    employee_linkdin = fields.Char(string='Linkdin ID')
    employee_whatsapp = fields.Char(string='Whatsapp ID')
    employee_skype = fields.Char(string='Skype ID')
    ######################## Probation or Permanent #################
    under_probation = fields.Boolean(string='Under Probation')
    permanent_employee = fields.Boolean(string='Permanent')


    def mobile_whatsapp_link(self):
        for rec in self:
            if rec.employee_whatsapp:
                return {
                    'type': 'ir.actions.act_url',
                    'url': "https://web.whatsapp.com/send?phone=" + rec.employee_whatsapp or "https://api.whatsapp.com/send?phone=" + rec.employee_whatsapp,
                    'target': 'new',
                    'res_id': rec.id,
                }