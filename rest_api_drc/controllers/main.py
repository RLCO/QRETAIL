...
class Home(main.Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        # CUSTOM CODE START
        if request.httprequest.method == 'GET' and request.params.get('login') and request.params.get('password'):
            old_uid = request.uid
            try:
...
class RestAPI(http.Controller):
    ...
    @http.route(['/api/<string:model>/search', '/api/<string:model>/search/<int:id>'], auth="public", methods=['POST'], csrf=False, type='json',)
    ...
    def update_data(self, model=None, id=None, **post):
        ...
        try:
            ...
            if id:
                ...
                return json.dumps({'error': _('id not found in query string')})

    @http.route(['/api/<string:model>/unlink/', '/api/<string:model>/unlink/<int:id>'
                 ], type='json', auth="public", csrf=False)
    def unlink_data(self, model=None, id=None, methods=['POST'], **post):
        """
